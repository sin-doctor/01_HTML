function gender(){
    ...
    switch(gdr){//어떤 조건이 도착했을 때 
        case "남성" : // 조건에 부합하는게 남성일때 
         alert("남성을 입력했네요!"); break;    
         //break; 작성해줄 것!
         /*
         case뒤에 break 문이나 return문을 작성하지 않을면 
         뒤에 case 들이 의미가 사라짐
         
         예를 들어 
         case "남성" 에 부합하는 조건을 실행한 후 확인해보면
         case "남성" 밑에 있는 case와 default의 구분에 관계 없이 
         여성과 default에 작성된 모든 코드들이 실행됨
         그래서 break 나 return 문을 작성해서 
         조건에 부합하는 case를 실행 후 switch 문을 탈출해야함.*/
        case "여성" : 
         alert("여성을 입력했네요!"); break;
        default :
         alert("올바른 성별을 입력해주세요. 남성 / 여성 "); break;
...
function order(){
    let menu;
    // 아래 menu는 위 변수명 menu에 들어값 값을 사용한다는 의미
